...
//CODIGO (comentado aquí porque no se puede comentar dentro de el __asm())
//    .substract
//    SUBS x, x, y //Restamos Y a X y lo guardamos en X, y se activan flags que se usan luego en el branch
//    ADD z, z, #1 //Sumamos 1 al contador, y no se activan flags
//    BHI substract //Branch, va a substract si x sigue siendo mayor que y





int my_division(int x, int y)
{
...
void lcd_display_dec(uint16_t value)
{
  if (value > 9999) {
    ...
    lcd_set(my_division(value, 1000), 1);
    lcd_set(my_division(value-(1000*my_division(value, 1000)), 100), 2);
    lcd_set(my_division(value -(100*my_division(value, 100)), 10), 3);
    lcd_set(value-(10*my_division(value, 10)), 4);
  }
}*/
...
void lcd_display_dec(uint16_t value)
{
  if (value > 9999) {
    ...
    lcd_set(my_division(value, 1000), 1);
    lcd_set(my_division(value, 100)-(10*my_division(value, 1000)), 2);
    lcd_set(my_division(value, 10)-(10*my_division(value, 100)), 3);
    lcd_set(my_division(value, 1)-(10*my_division(value, 10)), 4);
  }
}
...
void lcd_display_time(uint8_t value1, uint8_t value2)
{
  if ((value1 > 99) | (value2 > 99)) {
   ...
   lcd_display_error(0x10);
    ...
    lcd_set(my_division(value1, 10), 1);
    lcd_set(value1 % 10, 2);
    lcd_set(my_division(value2, 10), 3);
    lcd_set(value2 % 10, 4);
    SegLCD_Col_On();
  }
}
